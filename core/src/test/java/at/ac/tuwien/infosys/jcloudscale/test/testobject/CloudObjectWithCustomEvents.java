...
@CloudObject
public class CloudObjectWithCustomEvents {
	
	@EventSink
	private IEventSink sink;
	
	@DestructCloudObject
	public void triggerEvent(String string) throws JMSException {
		
		MyCustomEvent customEvent = new MyCustomEvent();
		customEvent.setCustomField(string);
		sink.trigger(customEvent);
