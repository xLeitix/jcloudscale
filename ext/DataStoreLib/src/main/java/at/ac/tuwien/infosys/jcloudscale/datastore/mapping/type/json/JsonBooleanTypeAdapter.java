...
public class JsonBooleanTypeAdapter implements TypeAdapter<Boolean, JsonElement> {

    @Override
    public JsonElement serialize(Boolean object, TypeMetadata<JsonElement> typeMetadata) {
        return new JsonPrimitive(object);
    }

    @Override
    public Boolean deserialize(JsonElement element, TypeMetadata<JsonElement> typeMetadata) {
        JsonPrimitive jsonPrimitive = (JsonPrimitive) element;
        if(!jsonPrimitive.isBoolean()) {
            throw new DatastoreException("Invalid value for boolean type.");
        }
