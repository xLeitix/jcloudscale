...
public class HbaseCharacterTypeAdapter implements TypeAdapter<Character, HbaseCell> {

    @Override
    public HbaseCell serialize(Character object, TypeMetadata<HbaseCell> typeMetadata) {
    ...
    @Override
    public Character deserialize(HbaseCell element, TypeMetadata<HbaseCell> typeMetadata) {
        byte[] value = element.getValue();
        return Bytes.toString(value).trim().charAt(0);
    }
}
