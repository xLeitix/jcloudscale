...
public class HbaseEnumTypeAdapter implements TypeAdapter<Enum, HbaseCell> {

    @Override
    public HbaseCell serialize(Enum object, TypeMetadata<HbaseCell> typeMetadata) {
        String className = typeMetadata.getParent().getClass().getSimpleName();
        return new HbaseCell(className, typeMetadata.getFieldName(), Bytes.toBytes(object.name()));
    }

    @Override
    public Enum deserialize(HbaseCell element, TypeMetadata<HbaseCell> typeMetadata) {
        byte[] value = element.getValue();
        String valueName =  Bytes.toString(value);
